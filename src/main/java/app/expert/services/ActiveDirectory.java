...
public class ActiveDirectory {
    ...
    private static String[] userAttributes = {
            ...
            "userPrincipalName", "department", "ipphone", "pager"
    };
    ...
    public static LdapContext getConnection(String username, String password, String domainName, String serverName) throws NamingException {
        ...
        try {
            ...
            throw new NamingException("Failed to connect to "
                    + domainName + ((serverName == null) ? "" : " through " + serverName));
        ...
        try {
            ...
            if (domainName != null) {
                ...
                if (answer.hasMore()) {
                    ...
                    if (user != null) return new User(attr, result);
                }
            }
        } catch (NamingException e) {
            //e.printStackTrace();
        ...
        if (authenticatedUser.contains("@")) {
            ...
            try {
                while (answer.hasMore()) {
                    ...
                    Attribute user = attr.get("userPrincipalName");
                    if (user != null) {
                        users.add(new User(attr, result));
                    }
                }
            } catch (Exception e) {
            }
    ...
    public static class User {
        ...
        public User(Attributes attr, SearchResult result) throws javax.naming.NamingException {
            ...
            surname = (String) attr.get("sn").get();
            distinguishedName = (String) attr.get("distinguishedName").get();
            ipPhone = (String) attr.get("ipPhone").get();
            pager = (String) attr.get("pager").get();
            ou = Arrays.stream(result.getName().split(","))
                    .filter(s -> s.startsWith("OU="))
                    .map(s -> s.replaceAll("OU=", ""))
