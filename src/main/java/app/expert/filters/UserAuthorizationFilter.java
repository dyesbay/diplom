...
//        this.roleCache = roleCache;
    ...
    private Claims parseSystemClaims(String token) throws GUnauthorized {
        return JwtUtils.getSystemClaims(token);
    }
    ...
    private void injectClaimsToContext(Claims claims) throws GBadRequest {
        ...
        if (claims.containsKey(GConstants.HEADER_AGENT)) {
            context.setAgentId(claims.get(GConstants.HEADER_AGENT, String.class).trim());
        }
        if (claims.containsKey(GConstants.HEADER_SUBJECT)) {
            String subject = claims.get(GConstants.HEADER_SUBJECT, String.class);
            if (!subject.equals(context.getSystem())) throw new GBadRequest();
        }
        if (claims.containsKey(GConstants.HEADER_SYSTEM)) {
            String systemFromKey = claims.get(GConstants.HEADER_SYSTEM, String.class);
            if (!context.getSystem().equals(systemFromKey)) throw new GBadRequest();
        }
    }
    ...
    private void doFilterUser(HttpServletRequest request,
        ...
        try {
            ...
            context.setUsername(manager.getUsername());
            context.setAgentId(manager.getAgentId());
...
//            springSecurityContextAuth("anonymous");
            ...
            logger.info(ExceptionUtils.getShortLog(ex));
...
//        }
        ...
        springSecurityContextAuth(manager.getUsername());
        chain.doFilter(request, response); // переход на след фильтр
    }

    private void doFilterSystem(HttpServletRequest request,
                                HttpServletResponse response,
                                FilterChain chain)
            throws GException, IOException, ServletException {

        // достать из токена agentId и убедиться что он настоящий
        injectClaimsToContext(parseSystemClaims(request.getHeader(HttpHeaders.AUTHORIZATION)));
    ...
    protected void doFilterInternal(HttpServletRequest request,
                                    HttpServletResponse response,
                                    FilterChain chain) throws IOException, ServletException {
        if (context.getRequester() != null && context.getRequester().equals("system"))
            doFilterSystem(request, response, chain);
        else
